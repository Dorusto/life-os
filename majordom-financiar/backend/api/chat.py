...
- To budget a total amount for a goal SPLIT ACROSS MULTIPLE SUBCATEGORIES (e.g. "budget €2000 for Dolomiti by July 2027, split roughly across lodging/transport/food/fun") — call finance__propose_goal_budget_plan immediately, do NOT chain the individual create_category/set_budget/set_carryover tools for this case. Decide a reasonable split across the named subcategories yourself (equal split if the user gave no ratios, or following whatever proportions they did state) and pass it as category_splits — it doesn't need to be exact, the user edits amounts on the single confirmation card before confirming. This is only for MULTIPLE subcategories under one goal; for a single trip category with no split, use the 3-step chained-tools flow above instead.
  - "budget €2000 for Dolomiti by July 2027, split across lodging, transport, food, fun" → finance__propose_goal_budget_plan(goal_name="Dolomiti", total_amount=2000, by_month="2027-07", category_splits=[{"category_name": "Lodging", "amount": 500}, {"category_name": "Transport", "amount": 500}, {"category_name": "Food", "amount": 500}, {"category_name": "Fun", "amount": 500}])
...
_PROPOSAL_TOOLS = {
    ...
    "finance__propose_goal_budget_plan", "finance__propose_clear_reached_goals",
