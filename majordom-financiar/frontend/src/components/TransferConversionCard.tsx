...
export default function TransferConversionCard({ data, onConfirmed, onCancelled }: Props) {
  ...
  return (
    <Card label="Convert to transfer" className="max-w-[85%] rounded-bl-sm">
      <div className="space-y-3">
        <div>
          <p className="text-token-ink-3 text-sm">
            <span className="text-token-ink">{data.payee || 'Unnamed'}</span> · {data.date} · <span className="font-mono tabular-nums">{formatCurrency(data.amount)}</span>
          </p>
