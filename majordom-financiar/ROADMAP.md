...
| 2.9 | IBAN auto-detection for CSV transfers | Store `IBAN: NLxx...` in AB account `note` field (via chat tool `set_account_iban`). On CSV import: read account notes → build IBAN→account map → match against Counterparty column (ING: `Counterparty`/`Tegenrekening`) → auto-select transfer destination for GT rows. Zero SQLite. AB is source of truth for IBANs. |
