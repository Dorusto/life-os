...
<a id="190-ab-credential-storage"></a>
### #190 (AB setup wizard) — credentials become runtime-reloadable, encryption key is disk-generated not user-supplied

**Date:** 2026-09-11

**Context:** Connecting to Actual Budget required editing `.env` directly — `ActualBudgetConfig` (`backend/core/config/settings.py`) read `ACTUAL_BUDGET_URL`/`PASSWORD`/`SYNC_ID` from the environment exactly once, in `__post_init__`, at process import time (critical rule #4: config from the settings singleton, never `os.environ` directly). A wizard that lets the user enter/test/save a connection at runtime needed two things the existing pattern didn't have: a way for a saved connection to take effect without a container restart, and a place to put the password that isn't a plaintext `.env` file the user edited by hand. Both were left deliberately vague in the original spec (`docs/specs/ab-setup-wizard.md`, "key derived locally, not hardcoded") — no existing precedent in this codebase to fall back on (`JWT_SECRET` in `auth.py` is a hardcoded-fallback env string, not derived from anything; no `Fernet`/encryption-at-rest pattern existed for any other secret).

**Decision 1 — runtime reload, not a restart:** `ActualBudgetConfig.__post_init__` now loads env defaults, then checks for a DB-saved override and applies it if present (`_load_saved_overrides()`). A new public `reload_from_db()` re-runs both steps and is called by the wizard's save endpoint immediately after writing new credentials — the *already-running* process picks up the change on its very next `get_provider()` call, because `ActualBudgetProvider._client()` already constructs a fresh `ActualBudgetClient` from `settings.actual.*` on every call (no caching to invalidate). This extends the settings-singleton pattern rather than bypassing it — nothing new reads `os.environ` directly outside `ActualBudgetConfig` itself.
...
**Verification:** live-tested end to end against the local Actual Budget fixture container — all 4 connection outcomes (success + budget list, wrong password → `auth`, unreachable host → `connection`, valid server + unknown file → `file_not_found`), and the full save → `reload_from_db()` → `get_provider()` round-trip confirmed working in the same already-running process with no restart. Real actualpy source read (not the docstring) to confirm `Actual(base_url=, password=)` without a context manager already performs the login round-trip and raises `AuthorizationError`/`httpx.HTTPError` appropriately, and to confirm `list_user_files()`'s actual return shape. Test data (a synthetic category/transaction, the saved credential blob and key file) created and cleaned up afterward — the fixture stack was left exactly as found.

**Scope cut, filed separately:** the original spec's "AB connection lost — reconnect" fallback banner for a *later* disconnection needs its own app-wide error-detection mechanism that doesn't exist today; building one was judged separable from the first-run setup friction this issue was actually about. Filed as #254.

---

<a id="219-f26-structured-transaction-card"></a>
### #219/F26 (chat re-formats transaction lists inconsistently) — a new structured card, not a system-prompt instruction
...
<a id="112-annual-budget-pacing"></a>
### #112 (annual budget pacing) — Settings-based editable config, formula extrapolated from elapsed months, bell surfaced only when over pace
...
**Decision 3 — bell alert only when over pace, and only when configured; never nags an unconfigured user into setting it up.** `compute_pacing_status()` returns `None` when unconfigured, and every caller (Settings status, `get_pending_items()`, the chat tool) must treat that as "show nothing" — discovered through the Settings page itself, not forced via a notification. Matches this cluster's existing precedent (#41/#117) of proactive-but-not-naggy surfacing.

**Live-verified against the local fixture stack, formula checked by hand**: real config saved (annual income 60000, one real category tagged fixed, one tagged sinking-fund) → `GET /api/budget-pacing/status` returned `expected_by_now=41046.89, actual_by_now=43089.17, over_by=2042.28`, independently recomputed from the raw `get_budget_pacing_totals()` output and confirmed to match exactly. Chat tool returns the same numbers. `get_pending_items()` correctly includes the item when over pace and omits it entirely when re-saved with a higher income (`on_pace: true`).

**Two things found and fixed during review, before commit, not after:**
1. **UI gap:** the two category-tagging lists (fixed / sinking-fund) let the same category be toggled on in both at once — the backend's `if cat_id in fixed_ids: ... elif cat_id in sinking_ids: ...` silently prioritizes "fixed" on a collision, so a sinking-fund toggle could look accepted in the UI but be silently ignored in the calculation. Fixed: the two `Set<string>` toggle states are now mutually exclusive in `BudgetPacingPage()` itself (toggling a category into one list clears it from the other), verified live in both directions plus a `curl` round-trip confirming zero overlap can be persisted.
2. **A real blind spot in `scripts/check_provider_wiring.py`**, found by hand during diff review, not by the script itself: it only ever traced `client = get_provider(); client.foo()` in the *same file* — `backend/core/finance/budget_pacing.py`'s `compute_pacing_status(provider: FinanceProvider)` receives the provider as a function parameter one hop away from any `get_provider()` call, so `get_budget_pacing_totals()` went completely unseen (script still printed "70 methods OK", unchanged, after the new method was wired). The wiring itself was correct — confirmed by hand across all 3 layers — but the safety net that's supposed to make that mechanical, per #126/#148's whole reason for existing, had a real gap. Fixed the same session (script now also treats any parameter annotated `FinanceProvider` as a provider variable, file-wide, same precision tradeoff the existing `get_provider()`-assignment detection already accepts) — count went 70 → 71 correctly after the fix, confirming it actually closes the gap rather than just changing the number.

---

<a id="111-unprotected-goal-detection"></a>
### #111 (unprotected savings-goal detection) — re-scoped from "needs live bank sync" to "reuses the existing TARGET: note mechanism", conservative no-name-matching trigger
...
**`budget_pacing.get_config()` returning `None` (user never configured #112 at all) is treated as `sinking_fund_category_ids: []`, not "skip this check"** — deliberately verified in the delegation prompt and confirmed correct: an unconfigured Settings page is not evidence that anything is protected, it's the opposite.
