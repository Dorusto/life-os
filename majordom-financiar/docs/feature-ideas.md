...
| ~~Goose as MCP client for Majordom~~ | **Tested 2026-08-28, dropped.** Tried connecting Goose to an existing MCP server (Home Assistant) as a dry run before Majordom even has one. Hit a real, unresolved upstream bug: Goose's `streamable_http` extension auto-triggers OAuth discovery and ignores a configured static Authorization header when the server advertises OAuth (matches [block/goose#5107](https://github.com/block/goose/issues/5107)); the SSE transport that would've avoided this was fully removed from Goose, not just hidden. No concrete use case identified either — Majordom's own chat UI + the planned Hermes/Telegram access already cover what was needed. Not worth revisiting unless a specific cross-domain need shows up later. |
