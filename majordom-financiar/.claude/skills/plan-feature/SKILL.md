...
"Implement directly" (per step 7 / CLAUDE.md's collaboration rules) means Claude does the work instead of DeepSeek — it does not mean doing it inline and going silent on the user mid-session. Once the plan is confirmed, launch a Claude Code fork (`Agent` tool, `subagent_type: "fork"`) to do the actual implementation in the background, and keep talking with the user in the main conversation while it runs. Established 2026-08-29 after a live session where the user asked for exactly this mid-implementation.
...
- Explicitly tell the fork not to commit — the user reviews the diff and confirms it works before any commit, same as any other implementation (`CLAUDE.md`'s commit rule doesn't change just because a fork did the typing).
...
- **Always confirm a fork actually did the work before reading its report.** Found live 2026-09-11 (#254): a fork returned a "done" report — plausible-sounding, describing the exact plan back — after 0 tool calls and ~6 seconds, having made zero file changes. `git status`/`git diff --stat` caught it immediately. Check one of those (or the fork's own reported tool-use count, if visible) before trusting any fork's completion report, every time — not just when something looks off.
...
**Manual prompt-file path — fallback only**, for when the user wants to run DeepSeek himself or Claude Code is unavailable: save to `scripts/prompts/deepseek/NNN_desc.md` using the template below, then stop — the user runs it manually. Not the default when Claude itself is doing the delegating.
...
- **Any "the LLM ignored a clear system-prompt instruction" report (skipped a tool call, or called it then ignored the result) — check `temperature` on that LLM payload BEFORE writing a new bullet or a heavier mechanism (forced `tool_choice`, retry-on-no-tool-call).** #172's real root cause, found only after two rounds of prompt-bullet fixes didn't fully close it: `_stream_with_tools()`'s payload in `backend/api/chat.py` had no `temperature` field at all — the only LLM call in the codebase missing one (`vision_engine.py`/`csv_importer/detector.py` use `0.0`, `notification_service.py` uses `0.7`). No override meant the provider default applied (`1.0` for `deepseek/deepseek-chat` via OpenRouter) — high for a call that should be near-deterministic. Live-retesting the identical query 3x fresh went from 1-of-3 clean to 3-of-3 clean after adding `"temperature": 0.2`. A missing/high temperature reproduces exactly like a flaky-prompt bug (same input, different outcomes each time) and is far cheaper to rule out first. See `docs/decisions.md#172-fabrication-root-cause-missing-temperature`.
...
- **Any new `except Exception`/`except SomeError` handler whose body is just `pass`/`continue`/`return None` swallows the failure with no trace** — this project's documented bug pattern is overwhelmingly "failed silently" (architecture.md rules 12/14/15/17/21/22). Fixed as a one-off at least 3 times (`decisions.md#174-code-audit`, commit `091f6c8`, #217) before a real check existed. Checked mechanically now — `python3 scripts/check_silent_exceptions.py` (wired into the pre-commit hook) flags any handler with no adjacent `logger.debug/info/warning/error` call; add the log call with the reason it's safe to ignore, or `# silent-ok: <reason>` on the `except` line for the rare case where even a debug log is genuinely wrong (e.g. real per-row hot-loop cost).
